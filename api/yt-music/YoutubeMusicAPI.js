...
class YoutubeMusicAPI {
    ...
    async getSuggestions(input, lang = null) {
        const data = await this.apiRequest('/music/get_search_suggestions', { input, lang });
        return Parser.parseSuggestions(data);
    }

    async search(query, categoryName = null, lang = null) {
        const data = await this.apiRequest('/search', { query, params: getCategoryURI(categoryName), lang });
        return Parser.parseSearch(data, categoryName);
    }

    async getSongInfo(videoId, lang = null) {
        let data = await this.apiRequest('/next', { videoId, lang });
        data = Parser.parseSong(data);
        data.id = videoId;
        return data;
    }

    async getArtist(browseId, lang = null) {
        browseId = String(browseId);
        if (!browseId.startsWith('UC'))
            throw new Error('Invalid artist browse ID');
        let data = await this.apiRequest('/browse', { ...createBrowseContext('ARTIST', browseId), lang });
        data = Parser.parseArtist(data);
        data.id = browseId;
        data.albums.forEach(a => a.author = { id: browseId, name: data.name });
        data.albums.sort((a, b) => b.year - a.year);
        data.songs.forEach(s => s.author.id = browseId);
        return data;
    }

    async getAlbum(browseId, lang = null) {
        browseId = String(browseId);
        if (!browseId.startsWith('MPREb'))
            throw new Error('Invalid album browse ID');
        let data = await this.apiRequest('/browse', { ...createBrowseContext('ALBUM', browseId), lang });
        data = Parser.parseAlbum(data);
        data.id = browseId;
        return data;
    }

    async getPlaylist(browseId, lang = null) {
        browseId = String(browseId);
        if (!browseId)
        ...
        let data = await this.apiRequest('/browse', { ...createBrowseContext('PLAYLIST', browseId), lang });
    ...
    async getTrends(lang = null) {
        const data = await this.apiRequest('/browse', { lang });
